...
ExtractIconExA.argtypes = [LPCSTR, ctypes.c_int,
...
def CreateListView(hwnd_parent, control_id, x, y, width, height, processes, icons, paths):
    ...
    hwnd_listview = CreateWindowExA(0, b"SysListView32", b"",
                                    ...
                                    hwnd_parent,
                                    HMENU(control_id),
                                    ctypes.cast(GetWindowLongPtrA(
                                        hwnd_parent, GetWindowLong.HINSTANCE), HINSTANCE),
                                    None
                                    )
    ...
    for i in range(len(processes)):
        ...
        lvitem.iImage = i
        lvitem.lParam = LPARAM(ctypes.cast(
            ctypes.byref(processes[i]), ctypes.c_void_p).value)

        if ListView_InsertItemA(hwnd_listview, ctypes.byref(lvitem)) == -1:
            raise Exception("ListView_InsertItem")

        ListView_SetItemTextA(hwnd_listview, i, 1, b"%d" %
                              processes[i].th32ProcessID)
        ListView_SetItemTextA(hwnd_listview, i, 2, b"%d" %
                              processes[i].th32ParentProcessID)
        ListView_SetItemTextA(hwnd_listview, i, 3, b"%d" %
                              processes[i].cntThreads)
        ListView_SetItemTextA(hwnd_listview, i, 4,
                              ctypes.cast(paths[i], ctypes.c_char_p))
        ...
        while Process32Next(snapshot, ctypes.byref(entry)):
            try:
                ...
                if process_machine.value == current_process_machine.value:
                    ...
                    full_process_image_name = ctypes.create_string_buffer(
                        MAX_PATH)
                    full_process_image_name_length = DWORD(MAX_PATH)
                    QueryFullProcessImageNameA(process_handle, 0,
                                               ctypes.cast(
                                                   full_process_image_name, ctypes.c_char_p),
...
def InjectDLL(target_pid, filename_dll):
    ...
    thread_handle = CreateRemoteThread(target_handle, None, 0,
                                       ctypes.cast(
                                           target_LoadLibraryA, LPTHREAD_START_ROUTINE),
...
def WindowProc(hwnd, uMsg, wParam, lParam):
    ...
    if uMsg == WindowMessage.COMMAND:
        ...
        if control_id == INJECT_BUTTON:
            ...
            print("Injecting into PID %d, process at %s" %
                  (target_pid, dll_path))
        ...
        if control_id == SEARCH_BUTTON:
            ...
            if not result:
                ...
                hwnd_edit = GetDlgItem(hwnd, FILEPATH_EDIT)
                SendMessageA(hwnd_edit, WindowMessage.SETTEXT, WPARAM(
                    0), LPARAM(ctypes.cast(text_buffer, ctypes.c_void_p).value))
...
if __name__ == "__main__":
    ...
    try:
        icon = LoadImageA(hinstance, b"gear_icon.ico", 1,
